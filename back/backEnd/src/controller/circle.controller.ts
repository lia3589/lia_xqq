...
export class CircleController {
  ...
  async getCircles(@Body() body: any) {
    ...
    const userCircles = circles.filter((circle) => user.circleIds.includes(circle.id));
  ...
  async addCircle(@Body() data: { userId: number, circleId: number }) {
    const usersPath = path.resolve(__dirname, '../data/users.data.json');
    const users = JSON.parse(fs.readFileSync(usersPath, 'utf-8'));
    ...
    if (user) {
      ...
      if (!user.circlesIds.includes(data.circleId)) {
        ...
        fs.writeFileSync(usersPath, JSON.stringify(users, null, 2), 'utf-8');
        return { success: true, message: 'Circle added successfully' };
      } else {
